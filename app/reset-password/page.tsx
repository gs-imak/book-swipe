    ...
    <main className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h1 className="text-lg font-serif font-bold text-stone-900 dark:text-stone-100"> {t("reset_password.set_a_new_password")} </h1>
        </div>

        {!configured ? (
          <p className="text-sm text-stone-500"> {t("reset_password.accounts_are_not_enabled_on_this")} </p>
        ) : done ? (
          <div className="flex flex-col items-center gap-3 py-6">
            <Check className="w-10 h-10 text-emerald-500" aria-hidden="true" />
            <p className="text-sm text-stone-600 dark:text-stone-400 text-center"> {t("reset_password.password_updated_redirecting_you_to_bookswip")} </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-3">
            <label htmlFor="new-password" className="sr-only">{t("reset_password.new_password")}</label>
            <input
              id="new-password"
              type="password"
            ...
            />
            <label htmlFor="confirm-password" className="sr-only">{t("reset_password.confirm_new_password")}</label>
            <input
              id="confirm-password"
              type="password"
            ...
            >
              {loading && <Loader2 className="w-4 h-4 animate-spin" aria-hidden="true" />} {t("reset_password.update_password")} </button>
          </form>
        )}
      </div>
    </main>
